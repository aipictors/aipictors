...
export function PaintCanvas(props: Props) {
  ...
  // フィルターを適用する関数
  const applyFilter = () => {
    const canvas = props.isMosaicMode ? imageCanvasRef.current : brushCanvasRef.current
    if (!canvas) return
        ...
        <div className="flex w-16 flex-col bg-gray-800 p-2 md:w-20">
          ...
          {!props.isMosaicMode && (
            <Button
              className={cn(
                "mb-2 h-12 w-12 md:h-16 md:w-16",
                tool === "brush" ? "bg-blue-600 text-white" : "bg-gray-700 text-gray-300 hover:bg-gray-600",
              )}
              size="icon"
              variant="ghost"
          ...
          {!props.isMosaicMode && (
            <Button
              className={cn(
                "mb-2 h-12 w-12 md:h-16 md:w-16",
                tool === "lasso" ? "bg-blue-600 text-white" : "bg-gray-700 text-gray-300 hover:bg-gray-600",
              )}
              size="icon"
              variant="ghost"
          ...
          {props.isMosaicMode && (
            <Button
              className={cn(
                "mb-2 h-12 w-12 md:h-16 md:w-16",
                tool === "lasso-mosaic" ? "bg-blue-600 text-white" : "bg-gray-700 text-gray-300 hover:bg-gray-600",
              )}
              size="icon"
              variant="ghost"
          ...
          <Button
            className={cn(
              "mb-2 h-12 w-12 md:h-16 md:w-16",
              isFilterPanelOpen ? "bg-blue-600 text-white" : "bg-gray-700 text-gray-300 hover:bg-gray-600",
            )}
            size="icon"
            variant="ghost"
        ...
        <div className="flex flex-1 flex-col md:flex-row">
          <div
            className={cn(
              ...
              (tool === "select" || props.isMosaicMode) && isPanning && "cursor-grabbing",
            )}
            onMouseDown={(e) => {
            ...
            onTouchStart={(e) => {
              if ((tool === "select" || props.isMosaicMode) && e.touches.length === 1) {
                e.preventDefault()
          ...
          <div className="hidden w-80 flex-col bg-gray-800 p-4 md:flex">
            {/* プレビューエリア */}
            <div className="mb-6">
              <h3 className="mb-3 text-sm font-bold text-gray-300">プレビュー</h3>
            ...
            {isFilterPanelOpen && (
              <div className="space-y-4">
                ...
                {/* 明度 */}
                <div>
                  <label className="mb-2 block text-xs text-gray-400">明度: {brightness}%</label>
                  <Slider
                    value={[brightness]}
                ...
                {/* コントラスト */}
                <div>
                  <label className="mb-2 block text-xs text-gray-400">コントラスト: {contrast}%</label>
                  <Slider
                    value={[contrast]}
                ...
                {/* 彩度 */}
                <div>
                  <label className="mb-2 block text-xs text-gray-400">彩度: {saturation}%</label>
                  <Slider
                    value={[saturation]}
                ...
                {/* 色相 */}
                <div>
                  <label className="mb-2 block text-xs text-gray-400">色相: {hue}°</label>
                  <Slider
                    value={[hue]}
            ...
            {(tool === "brush" || tool === "eraser") && (
              <div className="mt-6">
                <label className="mb-2 block text-xs text-gray-400">ブラシサイズ: {brushSize}px</label>
                <Slider
                  value={[brushSize]}
