  ...
  const tagElements = useMemo(() => {
    ...
    return cloudTags.map((tag, index) => {
      ...
      const colors = [
        ...
        "bg-lime-50 text-lime-600 border border-lime-200/50",
    ...
    <section className="space-y-8">
      ...
      <div className="space-y-3 text-center">
        <div className="flex items-center justify-center gap-2">
          ...
          <h2 className="bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text font-semibold text-2xl text-transparent">
            {t("タグクラウド", "Tag Cloud")}
          </h2>
        </div>
      ...
      <div className="relative mx-auto max-w-2xl">
        <div className="relative aspect-square w-full overflow-hidden rounded-2xl bg-gradient-to-br from-white to-gray-50 p-6 shadow-sm dark:from-gray-900/10 dark:to-gray-800/20">
          {/* 背景装飾 */}
              ...
              >
                <Link to={`/tags/${element.tag.tagName}`} className="block">
                  ...
                  >
                    ...
                    {/* ホバー効果 - スケールなしのエフェクト */}
                    <div className="absolute inset-0 rounded-full bg-white/20 opacity-0 transition-opacity duration-200 group-hover:opacity-100" />
                  </div>
                </Link>
              </div>
            ))}
